...
// newModelAgent resolves the agent's model string to a provider-backed
// sdk.Agent. The dispatch itself lives in pkg/provider so every entry point
// resolves models identically.
func (a *engage) newModelAgent(agent config.Agent) sdk.Agent {
	modelAgent, err := provider.New(a.configFactory, agent)
	if err != nil {
